...
const CHANNELS = [
  { id: "tech_news", name: "Tech News Daily", icon: "💻", type: "channel", members: 124500, lastMsg: "New iPhone 17 leaked!", time: "10:32", unread: 5 },
  { id: "crypto", name: "Crypto Signals", icon: "₿", type: "channel", members: 89200, lastMsg: "BTC breaking 70k", time: "10:15", unread: 12 },
  { id: "sports", name: "Sports Live", icon: "⚽", type: "channel", members: 210000, lastMsg: "GOAL! 2-1 in 78th", time: "09:58", unread: 3 },
  { id: "memes", name: "Meme Zone", icon: "😂", type: "channel", members: 450000, lastMsg: "Code works first try", time: "09:45", unread: 0 },
  { id: "coding", name: "Dev Community", icon: "👨‍💻", type: "channel", members: 98000, lastMsg: "React 20 announced!", time: "08:55", unread: 8 },
];
const CHANNEL_MSGS = {
  tech_news: [{ id:1, text:"Apple announces iPhone 17 with AI chip", time:"09:00", views:45200 },{ id:2, text:"Google DeepMind beats GPT-4 on all benchmarks", time:"09:30", views:38100 }],
  crypto: [{ id:1, text:"BTC Analysis: Support at $68,500. Resistance: $72,000", time:"09:15", views:28900 },{ id:2, text:"BTC breaking 70k resistance!", time:"10:15", views:8900 }],
  sports: [{ id:1, text:"LIVE: Man City vs Arsenal 1-1 (65)", time:"09:30", views:89000 },{ id:2, text:"GOAL! 2-1 in 78th minute!", time:"09:58", views:145000 }],
  memes: [{ id:1, text:"When the code works on first try\n*surprised pikachu*", time:"09:00", views:234000 }],
  coding: [{ id:1, text:"React 20 announced!\n- Server Components\n- Built-in state\n- 50% faster", time:"08:30", views:45000 }],
};
export default function Telegram() {
  ...
  const sendMessage = () => {
    ...
    setText("");
    if (!selected.type && user?._id) { const socket = getSocket(user._id); socket.emit("sendMessage", { receiverId: selected._id, text: msg.text }); }
  };
  const openChat = (item) => {
    setSelected(item); setView("chat"); setSearch(""); setSearchResults([]);
    if (!item.type && !recentDMs.find(d => d._id === item._id)) setRecentDMs(prev => [item, ...prev]);
  };
  const allChats = [...CHANNELS, ...recentDMs.map(d => ({ ...d, id: d._id, type: "dm", lastMsg: dmMessages[d._id]?.slice(-1)[0]?.text || "No messages", time: "", unread: 0 }))];
  const filtered = allChats.filter(c => tab === "channels" ? c.type === "channel" : tab === "dms" ? c.type === "dm" : true);
    ...
    return (
      <div className="flex flex-col h-[calc(100dvh-56px)] md:h-screen" style={{ background: "#0e1621" }}>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2 scrollbar-hide" style={{ background: "#0e1621" }}>
          {msgs.length === 0 && <div className="flex flex-col items-center justify-center h-full gap-3 text-gray-500"><FaTelegram size={48} className="text-blue-400 opacity-30" /><p className="text-sm">No messages yet</p></div>}
          {msgs.map((msg, i) => (
        ...
        {!isChannel ? (
          <div className="flex items-center gap-2 px-3 py-3 flex-shrink-0" style={{ background: "#17212b" }}>
            ...
            <input value={text} onChange={e => setText(e.target.value)} onKeyDown={e => e.key === "Enter" && sendMessage()} placeholder="Message" className="flex-1 rounded-full px-4 py-2 text-sm text-white outline-none" style={{ background: "#0d1117", border: "1px solid #2b3a4a" }} />
            {text.trim() ? <button onClick={sendMessage} className="w-10 h-10 rounded-full flex items-center justify-center text-white" style={{ background: "#2b5278" }}><FaPaperPlane size={14} /></button> : <button className="w-10 h-10 rounded-full flex items-center justify-center text-gray-400 hover:text-white"><FaMicrophone size={16} /></button>}
          </div>
        ) : <div className="px-4 py-3 text-center text-xs text-gray-500 flex-shrink-0" style={{ background: "#17212b" }}>Channel — only admins can post</div>}
      </div>
    );
  }
  return (
    <div className="flex flex-col h-[calc(100dvh-56px)] md:h-screen relative" style={{ background: "#0e1621" }}>
      <div className="px-4 pt-4 pb-2 flex-shrink-0" style={{ background: "#17212b" }}>
        ...
        <div className="relative flex items-center gap-2 rounded-xl px-3 py-2 mb-3" style={{ background: "#0d1117" }}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search" className="flex-1 bg-transparent text-white text-sm outline-none placeholder:text-gray-500" />
            ...
            <div className="absolute left-0 right-0 top-full mt-1 rounded-xl shadow-2xl z-50 overflow-hidden" style={{ background: "#17212b" }}>
              {searchResults.map(u => (
                <div key={u._id} onClick={() => openChat(u)} className="flex items-center gap-3 px-4 py-3 hover:bg-white/5 cursor-pointer">
                  ...
                  <div><p className="text-sm font-semibold text-white">{u.username}</p><p className="text-xs text-gray-400">{u.name || "User"}</p></div>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="flex gap-1">
          {[["all","All"],["channels","Channels"],["dms","DMs"]].map(([id,label]) => (
            <button key={id} onClick={() => setTab(id)} className="px-4 py-1.5 rounded-full text-xs font-semibold transition" style={{ background: tab === id ? "#2b5278" : "transparent", color: tab === id ? "#fff" : "#9ca3af" }}>{label}</button>
          ))}
        </div>
      </div>
          ...
          return (
            <div key={key} onClick={() => openChat(chat)} className="flex items-center gap-3 px-4 py-3 cursor-pointer hover:bg-white/5 transition-colors">
              <div className="relative flex-shrink-0">
                ...
                {chat.type === "channel" && <div className="absolute -bottom-0.5 -right-0.5 w-5 h-5 rounded-full flex items-center justify-center" style={{ background: "#2b5278" }}><FaVolumeUp size={8} className="text-white" /></div>}
              </div>
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center justify-between gap-2 mt-0.5">
                  ...
                  {chat.unread > 0 && <span className="flex-shrink-0 min-w-[20px] h-5 rounded-full flex items-center justify-center text-[10px] font-bold text-white px-1" style={{ background: "#2b5278" }}>{chat.unread}</span>}
                </div>
              </div>
            </div>
          );
      ...
      <div className="absolute bottom-20 md:bottom-6 right-4">
        <button onClick={() => setSearch(" ")} className="w-14 h-14 rounded-full flex items-center justify-center text-white shadow-2xl hover:scale-105 transition" style={{ background: "#2b5278" }}><FaPaperPlane size={20} /></button>
      </div>
    </div>
  );
}
