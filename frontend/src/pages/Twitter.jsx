...
const av = (u) =>
  u?.avatar ||
  `https://ui-avatars.com/api/?name=${encodeURIComponent(u?.username || "U")}&background=1d9bf0&color=fff&bold=true`;
      ...
      <div className="flex items-center gap-3 px-4 py-2.5">
        ...
        <div className={`flex-1 flex items-center gap-2 ${c.bgInput} rounded-full px-4 py-2 border border-transparent focus-within:border-[#1d9bf0] transition`}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search"
            className={`bg-transparent ${c.text} text-[14px] outline-none flex-1`} />
        </div>
        <button onClick={toggleTheme} className={`p-2 rounded-full ${c.iconHover} transition flex-shrink-0 ${c.textMuted}`}>
            ...
            <div className="mt-3 flex gap-2 items-start">
              ...
              <div className={`flex-1 border ${c.border} rounded-2xl px-3 py-2`}>
                <input value={replyText} onChange={e => setReplyText(e.target.value)}
                  ...
                  className={`w-full bg-transparent ${c.text} text-[15px] outline-none`} />
    ...
    <div className="pt-2 space-y-4 px-4">
      ...
      <div className={`${c.bgCard} rounded-2xl overflow-hidden`}>
        ...
        {displayTrends.map((t, i) => (
          <div key={i} className={`px-4 py-3 ${c.bgHover} transition cursor-pointer border-t ${c.border} flex items-start justify-between group`}>
            <div>
...
export default function Twitter() {
  ...
  const loadFeed = useCallback(async (reset = false) => {
    ...
    try {
      ...
      let newPosts = [];
      if (tab === "following") {
        const res = await getFeed(p, true);
        newPosts = res.data?.posts || res.data || [];
      } else {
  ...
  return (
    <div className={`${c.bg} h-full overflow-hidden`}>
      <div className="w-full flex h-full">
        ...
        <main className={`flex-1 min-w-0 border-x ${c.border} flex flex-col h-full overflow-y-auto scrollbar-hide`}>
          ...
          {loading && posts.length === 0 ? (
            ...
            <div className="text-center py-16 px-8">
              ...
              <h2 className={`text-[23px] font-extrabold ${c.text} mb-2`}>Welcome to X!</h2>
        ...
        {/* Right sidebar */}
        <div className={`w-[340px] flex-shrink-0 h-full overflow-y-auto scrollbar-hide hidden lg:block ${c.bg}`}>
